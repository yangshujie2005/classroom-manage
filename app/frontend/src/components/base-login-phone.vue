...
const rules = ref({
  phone: {
    rules: [
      {required: true, errorMessage: proxy.$tt('login.enter_phone')},
      {format: 'phone', errorMessage: proxy.$tt('login.phone_format_error')}
    ]
  },
  code: {
    rules: [
...
const sendCode = () => {
  if (!form.value.phone) {
    return uni.showToast({title: proxy.$tt('login.enter_phone'), icon: 'none'});
  }
  proxy.$refs.formRef.validateField('phone').then(res => {
    proxy.$cf.login.sendPhoneCode({phone: form.value.phone})
        .then((res) => {
...
const submitForm = async (formEl) => {
  ...
  await proxy.$refs.formRef.validate().then(res => {
    ...
    proxy.$cf.login.loginByPhoneCode(form.value).then((res) => {
      login_success(res);
    }).catch(err => {
      console.error(err)
      login_error(err);
...
function login_setting() {
  proxy.$cf.setting.login().then((res) => {
    ...
    if (!shouldShow('login.show.phone', configList)) {
