...
<booking_info_component
 :params="{bookingStatusEnumBookingStatusEnumId1:routerQuery.bookingStatusEnumId}"
 />
</el-tab-pane>
</el-tabs>
</base-wrapper>
  ...
  proxy.$refs.dataFormRef.validate(async (valid) => {
    if (valid) {
      ...
      if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
          ...
          let res = await proxy.$api.booking_status_enum['update'](form.value);
          proxy.$modal.msgSuccess(res.message);
          if(res.code == 0){
            refresh();
          }
...
async function getDetail(id) {
    if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
        ...
        let res = await proxy.$api.booking_status_enum['get'](id);
        if(res.code == 0){
            form.value = Object.assign({}, res.data);
        }
        else{
...
function refresh(){
    if(routerQuery.bookingStatusEnumId){
        getDetail(routerQuery.bookingStatusEnumId);
    }
    if(detail.value == 'all'){
        if(routerQuery.bookingStatusEnumId){
            getDetailAll(routerQuery.bookingStatusEnumId);
        }
    }
}
