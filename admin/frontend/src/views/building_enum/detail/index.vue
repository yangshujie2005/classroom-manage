...
<div class="flex-between-start" style="background-color: #5D5FEF; height: 140px;">
    <div class="flex-c-center-start h-full">
      <div class="m-x-20">
          <span style="font-weight: bold; font-size: 20px; color: #FFFFFF;">教学楼枚举</span>
      </div>
      <div class="m-x-20 m-t-4">
        <span style="font-size: 14px; color: #FFFFFF;">主键、教学楼名称。</span>
      </div>
    </div>
...
label="查看教室信息"
>
<classroom_info_component
 :params="{buildingEnumBuildingEnumId1:routerQuery.buildingEnumId}"
 />
</el-tab-pane>
</el-tabs>
</base-wrapper>
  ...
  proxy.$refs.dataFormRef.validate(async (valid) => {
    if (valid) {
      ...
      if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
          ...
          let res = await proxy.$api.building_enum['update'](form.value);
          proxy.$modal.msgSuccess(res.message);
          if(res.code == 0){
            refresh();
          }
...
async function getDetail(id) {
    if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
        ...
        let res = await proxy.$api.building_enum['get'](id);
        if(res.code == 0){
            form.value = Object.assign({}, res.data);
        }
        else{
...
function refresh(){
    if(routerQuery.buildingEnumId){
        getDetail(routerQuery.buildingEnumId);
    }
    if(detail.value == 'all'){
        if(routerQuery.buildingEnumId){
            getDetailAll(routerQuery.buildingEnumId);
        }
    }
}
