...
<div class="flex-between-start" style="background-color: #5D5FEF; height: 140px;">
    <div class="flex-c-center-start h-full">
      <div class="m-x-20">
          <span style="font-weight: bold; font-size: 20px; color: #FFFFFF;">背景图片</span>
      </div>
      <div class="m-x-20 m-t-4">
        <span style="font-size: 14px; color: #FFFFFF;">主键、上传管理员、背景图片、上传时间。</span>
      </div>
    </div>
...
label-width='150px'
>
    <el-form-item   label="上传管理员" prop="userInfoUserInfoId1">
  ...
  proxy.$refs.dataFormRef.validate(async (valid) => {
    if (valid) {
      ...
      if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
          ...
          let res = await proxy.$api.background_image['update'](form.value);
          proxy.$modal.msgSuccess(res.message);
          if(res.code == 0){
            refresh();
          }
...
async function getDetail(id) {
    if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
        ...
        let res = await proxy.$api.background_image['get'](id);
        if(res.code == 0){
            form.value = Object.assign({}, res.data);
        }
        else{
...
async function getDetailAll(id){
    if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
        ...
        let res = await proxy.$api.background_image_all['get'](id);
        if(res.code == 0){
            formAll.value = Object.assign({}, res.data);
        }
        else{
...
function refresh(){
    if(routerQuery.backgroundImageId){
        getDetail(routerQuery.backgroundImageId);
    }
    if(detail.value == 'all'){
        if(routerQuery.backgroundImageId){
            getDetailAll(routerQuery.backgroundImageId);
        }
    }
}
