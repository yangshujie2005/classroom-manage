...
<div class="flex-between-start" style="background-color: #5D5FEF; height: 140px;">
    <div class="flex-c-center-start h-full">
      <div class="m-x-20">
          <span style="font-weight: bold; font-size: 20px; color: #FFFFFF;">报修状态</span>
      </div>
...
<repair_order_component
 :params="{repairStatusEnumRepairStatusEnumId1:routerQuery.repairStatusEnumId}"
 />
</el-tab-pane>
</el-tabs>
</base-wrapper>
  ...
  proxy.$refs.dataFormRef.validate(async (valid) => {
    if (valid) {
      ...
      if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
          ...
          let res = await proxy.$api.repair_status_enum['update'](form.value);
          proxy.$modal.msgSuccess(res.message);
          if(res.code == 0){
            refresh();
          }
...
async function getDetail(id) {
    if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
        ...
        let res = await proxy.$api.repair_status_enum['get'](id);
        if(res.code == 0){
            form.value = Object.assign({}, res.data);
        }
        else{
...
function refresh(){
    if(routerQuery.repairStatusEnumId){
        getDetail(routerQuery.repairStatusEnumId);
    }
    if(detail.value == 'all'){
        if(routerQuery.repairStatusEnumId){
            getDetailAll(routerQuery.repairStatusEnumId);
        }
    }
}
