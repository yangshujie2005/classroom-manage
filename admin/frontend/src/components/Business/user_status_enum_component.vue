...
@addEvent="handleAdd"
uploadExcelAPI="user_status_enum.import"
...
:query="{userStatusEnumId: scope.row.userStatusEnumId}"
text="详情"
>
</base-info-btn>
<base-edit-btn
...
function handleSelectionChange(selection) {
    multipleSelection.value = selection.map(item => item.user_status_enum_id);
}
...
async function handleDelete(row) {
  if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
    let res = await proxy.$api.table.delete({
        table_name: 'user_status_enum',
        param: {
            userStatusEnumId: row.userStatusEnumId
        }
    });
    proxy.$modal.msgSuccess(res.message);
  }
  else{
    let res = await proxy.$api.user_status_enum.delete(row.userStatusEnumId);
    proxy.$modal.msgSuccess(res.message);
  }
  refreshTableData();
}
...
async function handleBatchDelete() {
    ...
    res = await proxy.$api.table.deleteBatch({
        ...
        res = await proxy.$api.user_status_enum.deleteBatch(ids);
    }
...
async function handleExport() {
  try {
    await proxy.$api.user_status_enum.downloadExcel({
      ...listQuery.value
      ...
      proxy.$refs.dataFormRef.validate(async (valid) => {
        if (valid) {
          if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
            ...
            let res = await proxy.$api.user_status_enum[dialogStatus.value](form.value);
            proxy.$modal.msgSuccess(res.message);
          }
          refreshTableData();
          dialogVisible.value = false;
