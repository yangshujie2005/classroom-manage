...
@addEvent="handleAdd"
uploadExcelAPI="repair_status_enum.import"
...
:query="{repairStatusEnumId: scope.row.repairStatusEnumId}"
text="详情"
>
</base-info-btn>
<base-edit-btn
...
function handleSelectionChange(selection) {
    multipleSelection.value = selection.map(item => item.repair_status_enum_id);
}
...
async function handleDelete(row) {
  if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
    let res = await proxy.$api.table.delete({
        table_name: 'repair_status_enum',
        param: {
            repairStatusEnumId: row.repairStatusEnumId
        }
    });
    proxy.$modal.msgSuccess(res.message);
  }
  else{
    let res = await proxy.$api.repair_status_enum.delete(row.repairStatusEnumId);
    proxy.$modal.msgSuccess(res.message);
  }
  refreshTableData();
}
...
async function handleBatchDelete() {
    ...
    res = await proxy.$api.table.deleteBatch({
        ...
        res = await proxy.$api.repair_status_enum.deleteBatch(ids);
    }
...
async function handleExport() {
  try {
    await proxy.$api.repair_status_enum.downloadExcel({
      ...listQuery.value
      ...
      proxy.$refs.dataFormRef.validate(async (valid) => {
        if (valid) {
          if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
            ...
            let res = await proxy.$api.repair_status_enum[dialogStatus.value](form.value);
            proxy.$modal.msgSuccess(res.message);
          }
          refreshTableData();
          dialogVisible.value = false;
