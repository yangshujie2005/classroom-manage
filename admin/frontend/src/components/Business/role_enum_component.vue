...
@addEvent="handleAdd"
uploadExcelAPI="role_enum.import"
...
:query="{roleEnumId: scope.row.roleEnumId}"
text="详情"
>
</base-info-btn>
<base-edit-btn
...
async function handleDelete(row) {
  if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
    let res = await proxy.$api.table.delete({
        table_name: 'role_enum',
        param: {
            roleEnumId: row.roleEnumId
        }
    });
    proxy.$modal.msgSuccess(res.message);
  }
  else{
    let res = await proxy.$api.role_enum.delete(row.roleEnumId);
    proxy.$modal.msgSuccess(res.message);
  }
  refreshTableData();
}
...
async function handleExport() {
  try {
    await proxy.$api.role_enum.downloadExcel({
      ...listQuery.value
      ...
      proxy.$refs.dataFormRef.validate(async (valid) => {
        if (valid) {
          if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
            ...
            let res = await proxy.$api.role_enum[dialogStatus.value](form.value);
            proxy.$modal.msgSuccess(res.message);
          }
          refreshTableData();
          dialogVisible.value = false;
